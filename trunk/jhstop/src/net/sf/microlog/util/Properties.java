 ...
 * Properties.java
 *
 * Created on den 20 oktober 2005, 10:11
 *
...
/**
 * A class that makes the properties available to the system. The
 * properties are based on key/value, i.e:
 * <code>some.key=some_value</code>
 * 
 * The properties are taken from the following sources in this order.
 * (That is, the values have precedence in this order.)
 *
 * Sources:
 * --------
 * - default values
 * - property text-file included in the JAR-file
 * - values from application properties (MIDlet.getAppProperty)
 * - values saved in rms
 *
 * (Example: keys/values saved in the rms overwrites keys/values from all 
 * other sources.)
 *
 * @author Darius Katz
 */
public class Properties implements PropertiesGetter {
    ...
    /**
	 * Creates a new properties object. Since no midlet is passed
     * as a parameter properties will not come from the application
     * properties.
	 */
    public Properties() {
        props = new Hashtable();
     ...
     *
     * @param m the MIDlet from which application properties will be fetched
	 */
     ...
     * PropertySources in a determined order. This order determines the
     * way in which values are overridden.
     *
