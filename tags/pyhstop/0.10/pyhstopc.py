...
class socketListener:
	...
	def __init__(self, lid = -1):
		self.lid = lid
		self.t = options.forward[self.lid][3]
...
class tunnelClient:
	...
	def __init__(self, queues, tid = -1):
		...
		(f00, self.destHost, self.destPort, self.destType) = options.forward[tid]
		...
		if self.head:
			self.cGET.setopt(self.cGET.HTTP0.10ER, self.head._make_headers())
			self.cPUT.setopt(self.cPUT.HTTP0.10ER, self.head._make_headers())
